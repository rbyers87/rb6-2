...
<input
  type="number"
  placeholder="Weight"
  value={exercise.weight || ''} // Display weight in kilograms (or leave blank if no weight)
  onChange={(e) => {
    const weight = Number(e.target.value); // Get the raw weight value entered (in kilograms)
    handleExerciseChange(index, "weight", weight); // Update state with the raw weight value entered
  }}
  className="block w-1/4 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
/>
